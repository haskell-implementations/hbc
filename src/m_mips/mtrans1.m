...
||  mt HSV (Mcomparesf a1 a2.ms) =
	flush HSV @ let HSV = (0,0,0) in
	let HSV' = chg 1 HSV a1 in
	Mcomparesf (adj 1 HSV a1) (adj 1 HSV' a2) .mt (chg 1 HSV' a2) ms
...
and Mmovey op x y =
	...
	Mmove x y
and tmp4 op = if ssize op = 2 then Ftmp4 else dtmp4
#endif
...
#if 0
and mv [] = []
||  mv (Mop3 op popV a2 a3.ms) = let tmp = tmp4 op in Mmovex op popV tmp.mv (Mop3 op tmp a2 a3.ms)
||  mv (Mop3 op a1 popV pushV.ms) = Mop3 op a1 (Vind 0) (Vind 0).mv ms
||  mv (Mop3 op a1 a2 pushV.ms) = let tmp = tmp4 op in mv (Mop3 op a1 a2 tmp.Mmovey op tmp pushV.ms)
||  mv (Mop3 op a1 popV a3.ms) = let tmp = tmp4 op in Mmovex op popV tmp.mv (Mop3 op a1 tmp a3.ms)
||  mv (Mop2 op popV a2.ms) = let tmp = tmp4 op in Mmovex op popV tmp.mv (Mop2 op tmp a2.ms)
||  mv (Mcompare popV a2.ms) = Mmove popV dtmp4.mv (Mcompare dtmp4 a2.ms)
||  mv (Mcompare a1 popV.ms) = Mmove popV dtmp4.mv (Mcompare a1 dtmp4.ms)
||  mv (Mcomparedf popV a2.ms) = Mmovedf popV Ftmp4.mv (Mcomparedf Ftmp4 a2.ms)
||  mv (Mcomparedf a1 popV.ms) = Mmovedf popV Ftmp4.mv (Mcomparedf a1 Ftmp4.ms)
