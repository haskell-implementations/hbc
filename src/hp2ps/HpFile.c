...
static int gotjob = 0;				/* "JOB" read	        */
static int gotdate = 0;				/* "DATE" read          */
static int gotvalueunit = 0;			/* "VALUE_UNIT" read    */
static int gotsampleunit = 0;			/* "SAMPLE_UNIT" read   */
static int insample = 0;			/* true when in sample  */

static float lastsample;			/* the last sample time */

static void GetHpLine();			/* forward	*/
static void GetHpTok(); 			/* forward	*/

static struct entry* GetEntry();		/* forward	*/

static void MakeIdentTable();			/* forward	*/
...
float *samplemap;		/* sample intervals	*/
float *markmap;			/* sample marks		*/
...
void StoreSample();			/* forward */
char* TokenToString();			/* forward */
...
static void GetHpLine(infp)
FILE* infp;
{
    static int nmarkmax = 0, nsamplemax = 0;
	...
	if (nmarks >= nmarkmax) {
	    if (!markmap) {
		nmarkmax = N_MARKS;
		markmap = (float*) xmalloc(nmarkmax * sizeof(float));
	    } else {
		nmarkmax *= 2;
		markmap = (float*) xrealloc(markmap, nmarkmax * sizeof(float));
	    }
	}
	markmap[ nmarks++ ] = thefloat; 
        GetHpTok(infp);
        break;
	...
	if (nsamples >= nsamplemax) {
		...
		samplemap = (float*) xmalloc(nsamplemax * sizeof(float));
	    } else {
		nsamplemax *= 2;
		samplemap = (float*) xrealloc(samplemap, 
	                                      nsamplemax * sizeof(float));
	    }
	}
	samplemap[ nsamples ] = thefloat;
	GetHpTok(infp);
	break;
    case END_SAMPLE_TOK: 
	insample = 0;
	...
	if (thetok != INTEGER_TOK) {
        ...
        StoreSample(GetEntry(theident), nsamples, (float) theinteger);
	GetHpTok(infp); 
        break;
    case EOF_TOK:
        endfile = 1;
 ...
 *	or "thefloat" as appropraite; in the case of identifiers 
 *	it is assigned to "theident".
 */

token GetNumber();			/* forward	*/
void  GetIdent();			/* forward	*/
void  GetString();			/* forward	*/
int   IsIdChar();			/* forward	*/

static void GetHpTok(infp)
FILE* infp;
{
...
int containsdot;
    ...
    if (containsdot) {
        thefloat = (float) atof(numberstring);
	return FLOAT_TOK;
    } else {
...
static struct entry* GetEntry(name)
char* name;
{
int h;
struct entry* e;
...
void StoreSample(en, bucket, value)
struct entry* en; int bucket; float value;
{
struct chunk* chk; 
