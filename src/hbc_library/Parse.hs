module Parse(Parser(..), (+.+), (..+), (+..), (|||), (>>), (>>>), (||!), (|!!), (.>),
	     into, lit, litp, many, many1, succeed, fail, sepBy, count, sepBy1, testp, token, recover,
...
data FailAt a
	= FailAt Int{-#STRICT#-} [PErrMsg] a		 	-- token pos, list of acceptable tokens, rest of tokens
	deriving (Text)
...
p `sepBy1` q = p `into` (\v-> many (q ..+ p) >> (v:))		-- p +.+ many (q ..+ p) >>> (:)    is slower
